...
2. 使用场景：
    ...
    * **双重检查锁**：在双重检查锁定（Double-Checked Locking）的实现中，需要使用volatile关键字来保证变量的可见性和顺序性。
3. `volatile`关键字的使用场景：
    1.  写入变量不依赖变量的当前值时。因为如果依赖当前值，将是获取-计算-写入这三步操作，这三步操作不 原子性的，而`volatile`不保证原子性。
    2. 读写变量值时，因为加锁本身已经保证了内存可见性，这时候不需要把变量声明为`volatile`的。                                                                                                                                                                                                                                                                                                                         
