...
5. 在进行aof文件的重写时，配置不要去做同步。若配置为yes，可能会导致日志的丢失，建议配置为no
   ...
   >
   > 这意味着，当另一个孩子在存钱时，Redis的持久性与“appendfsync none”相同。实际上，这意味着在最坏的情况下(使用默认的Linux设置)可能会损失30秒的日志。
   >
